...
以下内容是项目完成后的定向优化，优先级高于原始条目：

1. 项目保持课程作业以上、生产系统以下的中等难度，不增加复杂工程化和对比实验。
2. 增加FastAPI入口，提供健康检查、询盘分析、产品查询和CRM读写五个基础接口。
3. 产品匹配增加品类关键参数硬约束，硬约束冲突不得判定为标准SKU。
4. 客户所在国、目标销售市场和交付地点分开提取；推断值必须给出商业提醒。
5. 将信息质量提醒、商业提醒和真实风险分层，只有真实风险强制人工复核。
6. 补充需求调研、解决方案和POC报告，并将面试辅导重点调整为岗位职责、模型应用、FastAPI和POC表达。
7. Streamlit仅作为AI辅助完成的演示界面，不要求掌握FastAPI以外的前后端技术。
